import { BlogPost } from '@/types/blog';

export const blogPosts: BlogPost[] = [
  {
    id: '1',
    title: 'Πώς να Δημιουργήσετε Viral Reels στη Θεσσαλονίκη το 2025',
    slug: 'viral-reels-thessaloniki-2025',
    excerpt: 'Ανακαλύψτε τα μυστικά για να δημιουργήσετε viral Instagram Reels που θα αυξήσουν την επισκεψιμότητα της επιχείρησής σας στη Θεσσαλονίκη.',
    content: `
# Πώς να Δημιουργήσετε Viral Reels στη Θεσσαλονίκη το 2025

Στη σημερινή ψηφιακή εποχή, τα **Instagram Reels** έχουν γίνει το πιο ισχυρό εργαλείο για digital marketing στη Θεσσαλονίκη. Ως leading digital agency, έχουμε βοηθήσει δεκάδες τοπικές επιχειρήσεις να αυξήσουν την online παρουσία τους μέσω strategically crafted Reels.

## Γιατί τα Reels είναι Κρίσιμα για την Επιχείρησή σας

Τα Instagram Reels έχουν **500% περισσότερο engagement** από τις κλασικές φωτογραφίες. Στη Θεσσαλονίκη, βλέπουμε συνεχώς local businesses να διπλασιάζουν τους followers τους σε λιγότερο από έναν μήνα.

### Τα 5 Βασικά Στοιχεία ενός Viral Reel

1. **Hook που Σε Κολλάει**: Τα πρώτα 3 δευτερόλεπτα καθορίζουν τα πάντα
2. **Trend Integration**: Χρησιμοποιούμε trending sounds και effects
3. **Local Connection**: Αναφορές στη Θεσσαλονίκη και τοπικές αναφορές
4. **Quality Production**: Professional lighting και editing
5. **Strategic Hashtags**: Mix από popular και niche hashtags

## Case Study: Local Restaurant Success

Πρόσφατα δουλέψαμε με ένα εστιατόριο στο κέντρο της Θεσσαλονίκης που είδε **300% αύξηση** στις κρατήσεις μετά από μια strategic Reel campaign που δημιουργήσαμε.

*Θέλετε να μάθετε περισσότερα για το πώς μπορούμε να βοηθήσουμε την επιχείρησή σας; Επικοινωνήστε μαζί μας σήμερα!*
    `,
    featuredImage: '/src/assets/portfolio-1.jpg',
    author: {
      name: 'Digital Marketing Team',
      avatar: '/src/assets/portfolio-1.jpg'
    },
    category: 'Digital Marketing',
    tags: ['Reels Θεσσαλονίκη', 'Instagram Marketing', 'Social Media', 'Viral Content'],
    publishedAt: '2025-01-15',
    readingTime: 5,
    seo: {
      metaTitle: 'Viral Reels Θεσσαλονίκη 2025 | Οδηγός Digital Marketing',
      metaDescription: 'Μάθετε πώς να δημιουργήσετε viral Instagram Reels στη Θεσσαλονίκη. Proven στρατηγικές από το leading digital agency της πόλης.',
      keywords: ['Reel Θεσσαλονίκη', 'viral reels', 'Instagram marketing Thessaloniki', 'social media marketing']
    }
  },
  {
    id: '2',
    title: 'Digital Agency Θεσσαλονίκη: Πώς να Επιλέξετε τη Σωστή Εταιρεία',
    slug: 'digital-agency-thessaloniki-guide',
    excerpt: 'Comprehensive οδηγός για την επιλογή της κατάλληλης digital agency στη Θεσσαλονίκη για την επιχείρησή σας.',
    content: `
# Digital Agency Θεσσαλονίκη: Πώς να Επιλέξετε τη Σωστή Εταιρεία

Η επιλογή της σωστής **digital agency στη Θεσσαλονίκη** είναι κρίσιμη για την επιτυχία της online παρουσίας της επιχείρησής σας. Με περισσότερες από 50 agencies στην πόλη, πώς ξεχωρίζετε την καλύτερη;

## Τι να Ψάχνετε σε μια Digital Agency

### 1. Proven Track Record
Ζητήστε case studies και concrete αποτελέσματα. Στο VerticalFlow, κάθε project έρχεται με detailed analytics και ROI measurements.

### 2. Local Market Knowledge  
Η Θεσσαλονίκη έχει unique characteristics. Μια good agency καταλαβαίνει το local market και το target audience.

### 3. Full-Service Capabilities
- **Web Development** με modern technologies
- **SEO & Google Ads** optimization  
- **Social Media Management** με έμφαση στα Reels
- **Branding & Design** που ξεχωρίζει

### 4. Transparent Communication
Regular reports, clear timelines, και open communication channels είναι must-haves.

## Red Flags να Αποφύγετε

❌ Υπερβολικές υποσχέσεις χωρίς data  
❌ Lack of local portfolio  
❌ Cheap pricing χωρίς explanations  
❌ Poor communication από την αρχή

## Γιατί το VerticalFlow Ξεχωρίζει

Ως η **leading digital agency στη Θεσσαλονίκη**, προσφέρουμε:

✅ **300+ Successful Projects** locally  
✅ **Average 250% ROI** για τους clients μας  
✅ **24/7 Support** στα ελληνικά  
✅ **Cutting-edge Technology** stack

*Ready να πάτε το business σας στο επόμενο level? Ας μιλήσουμε!*
    `,
    featuredImage: '/src/assets/portfolio-2.jpg',
    author: {
      name: 'Strategy Team',
      avatar: '/src/assets/portfolio-2.jpg'
    },
    category: 'Business Strategy',
    tags: ['Digital Agency Θεσσαλονίκη', 'Web Development', 'SEO', 'Business Growth'],
    publishedAt: '2025-01-10',
    readingTime: 7,
    seo: {
      metaTitle: 'Καλύτερη Digital Agency Θεσσαλονίκη | VerticalFlow Guide',
      metaDescription: 'Οδηγός επιλογής digital agency στη Θεσσαλονίκη. Μάθετε τι να ψάχνετε και πώς να αποφύγετε λάθη στην επιλογή σας.',
      keywords: ['Digital Agency Θεσσαλονίκη', 'web development Thessaloniki', 'digital marketing agency']
    }
  }
];

export const blogCategories = [
  {
    id: '1',
    name: 'Digital Marketing',
    slug: 'digital-marketing',
    description: 'Στρατηγικές και τακτικές για online marketing success',
    count: 12
  },
  {
    id: '2', 
    name: 'Web Development',
    slug: 'web-development',
    description: 'Modern web development techniques και best practices',
    count: 8
  },
  {
    id: '3',
    name: 'Business Strategy',
    slug: 'business-strategy', 
    description: 'Επιχειρηματικές στρατηγικές για digital transformation',
    count: 6
  }
];