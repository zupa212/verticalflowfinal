...
const Footer: React.FC = () => {
  ...
  const navigationItems = [
    ...
    { icon: Shield, label: 'Our Work', href: '#work' },
  ...
  return (
    <footer 
      ref={footerRef}
      className="relative bg-muted/30 border-t border-border section-separator"
    >
      {/* Grid Background */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            ...
            <div className="flex items-center gap-8 bg-background/50 backdrop-blur-sm border border-border rounded-full px-8 py-4 nav-glow">
              {navigationItems.map((item, index) => (
                <motion.a
                  key={item.label}
                  href={item.href}
                  className="flex flex-col items-center gap-1 hover-glow cursor-pointer group transition-all duration-300"
                  whileHover={{ y: -2 }}
                  whileTap={{ scale: 0.95 }}
                >
            ...
            >
              {socialItems.map((item, index) => (
                <motion.a
                  key={item.label}
                  href={item.href}
                  className="w-10 h-10 bg-muted/50 hover:bg-muted/70 rounded-full flex items-center justify-center hover-glow cursor-pointer group transition-all duration-300"
        ...
        <div className="mt-16 pt-8 border-t border-border">
          ...
          >
            <div className="text-4xl font-black text-foreground hover-glow cursor-pointer inline-block">
              kree8
            </div>
          </div>
        </div>
